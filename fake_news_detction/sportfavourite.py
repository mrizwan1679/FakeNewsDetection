...
sportfavourite_bp = Blueprint('goldfavourite', __name__)
...
@sportfavourite_bp.route('/save-favorite', methods=['POST'])
def save_favorite():
    ...
    category = data.get('category', 'Sports')  # Default to 'General'
...
@sportfavourite_bp.route('/get-news', methods=['GET'])
def get_news():
    if 'user' not in session:
        return jsonify({"error": "Unauthorized"}), 401
...
@sportfavourite_bp.route('/delete-favorite', methods=['POST'])
def delete_favorite():
    if 'user' not in session:
        return jsonify({"error": "Unauthorized"}), 401
